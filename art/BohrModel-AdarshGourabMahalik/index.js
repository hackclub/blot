...
function calculateMassNumber(atomicNumber) {
  return atomicNumber === 1
    ...
    : atomicNumber % 2 === 0
    ? 2 * atomicNumber
    : 2 * atomicNumber + 1; // Who knew atomic math could be so dramatic?
}
...
function drawCircle(radius) {
  t.up()
    .goTo([125, 125 - radius])
    .down(); // Turtle travels to the start point, because even turtles need directions
...
shellRadii
  .slice(0, shells.length)
  .forEach((radius, index) => drawCircle(radius));
