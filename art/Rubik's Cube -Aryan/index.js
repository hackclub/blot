...
function createRubiksCube() {
  ...
  faces.right.forEach(cell => {
    ...
    const [x1, y1, z1] = rotate3D(x - cubeSize / 2, y - cubeSize / 2, z, 0, -80, 0);
    const [x2, y2, z2] = rotate3D(x + cubeSize / 2, y - cubeSize / 2, z, 0, -80, 0);
    const [x3, y3, z3] = rotate3D(x + cubeSize / 2, y + cubeSize / 2, z, 0, -80, 0);
    const [x4, y4, z4] = rotate3D(x - cubeSize / 2, y + cubeSize / 2, z, 0, -80, 0);
    ...
    const translatedPolyline = polyline.map(([px, py]) => [px + cubeSize * translationFactor, py]);

    allPolylines.push({ points: translatedPolyline, color: cell.color });
  });

  // Translate and rotate the top face
  faces.top.forEach(cell => {
    const [x, y, z] = translate3D(cell.x, cell.y, cell.z, 0, cubeSize * translationFactor, 0);
    const [x1, y1, z1] = rotate3D(x - cubeSize / 2, y - cubeSize / 2, z, 86.5, -15, 1.5);
    const [x2, y2, z2] = rotate3D(x + cubeSize / 2, y - cubeSize / 2, z, 86.5, -15, 1.5);
    const [x3, y3, z3] = rotate3D(x + cubeSize / 2, y + cubeSize / 2, z, 86.5, -15, 1.5);
    const [x4, y4, z4] = rotate3D(x - cubeSize / 2, y + cubeSize / 2, z, 86.5, -15, 1.5);
    ...
    // Move the top face upwards
    const translatedPolyline = polyline.map(([px, py]) => [px+1.5, py + cubeSize *translationFactor-0.25 ]);
  ...
  allPolylines.forEach(polyline => {
    drawLines([polyline.points], { stroke: 'black', fill: polyline.color, width: 1 });
  });
}
