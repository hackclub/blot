...
const yellow = "#FFFF00"
const brown = "#7c3f00"
const blue = `#0000${((bt.randInRange(0.7, 1) * 255) | 0).toString(16).padStart(2, '0')}`

const sunRadius = bt.randIntInRange(2,20); 
const sunX = bt.randIntInRange(2 + sunRadius,123 - sunRadius)
const sunY = bt.randIntInRange(123 -sunRadius, 100- sunRadius)

const scale1 = bt.randInRange(0.6, 0.9)
const scale2 = bt.randInRange(0.5, 0.8)
const scale3 = bt.randInRange(0.4, 0.7)
...
// draw circle for sun
const sun = (center, radius, numPoints = 50) => {
  const points = [];
...
// draw various parts of cedar tree
...
const rows = (x , y) => {
  bt.translate(pieces, [0,10])
  // create Green, comment out for black and white
  const green = `#00${((bt.randInRange(0.5, 1) * 255) | 0).toString(16).padStart(2, '0')}00`;

  //create individual scales for each row
  for (let i = 0; i < pieces.length ; i++){
    bt.scale([pieces[i]], [x, y], [0,0])
  }
  //record width, height, and how many will fit 
  ...
  for (let i = 0; i < divisibleX; i++){
    //change to white for b/w
    redraw(green)
  ...
  bt.translate(pieces, [cwidth/2,cheight/2])
  for (let i = 0; i < pieces.length ; i++){
    bt.scale([pieces[i]], [1,1
                          ], [0,0])
  }
  return pieces;
  }

// UNUSED CODE, ONLY NEED TO CREATE COLUMNS
// const divisibleY = Math.floor(height / cheight) 
...
// comment out lines to create black and white
drawLines([sky], {fill: blue})
drawLines([ground], {fill: 'brown'})

// just delete fill for b/w
drawLines([sun([sunX,sunY],sunRadius)], {fill: yellow})
