...
// Generate a mosaic effect
function generateMosaic(lines, cellSize) {
  const mosaicLines = [];
  lines.forEach(line => {
    line.forEach(([x, y]) => {
      const newX = Math.floor(x / cellSize) * cellSize;
      const newY = Math.floor(y / cellSize) * cellSize;
...
const mosaicLines = generateMosaic(mirroredLines, bt.randIntInRange(3, 7)); // Random cell size

// Apply noise to mosaic lines
...
// Draw the mosaic with random colors
mosaicLines.forEach(line => {
  drawLines([line], {
    fill: getRandomColor(),
    stroke: "#fff",
