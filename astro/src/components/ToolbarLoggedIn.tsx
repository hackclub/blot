...
import type { PersistenceState } from '../lib/state.ts'
import { Signal, useSignal, useSignalEffect } from '@preact/signals'

const saveName = throttle(500, async (artId: string, newName: string) => {
  try {
    const res = await fetch('/api/art/rename', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ artId, newName })
    })
    if (!res.ok) throw new Error(`Error renaming piece: ${await res.text()}`)
  } catch (error) {
    console.error(error)
  }
})
...
const onNameEdit = (
  ...
  if (
    ...
    return
  saveName(persistenceState.value.art.id, newName)
...
const canDelete = (persistenceState: Signal<PersistenceState>) => {
  return (
    ...
    persistenceState.value.kind === 'PERSISTED' &&
    persistenceState.value.art !== 'LOADING' &&
    !persistenceState.value.art.unprotected
  )
}
...
export default function Toolbar(props: ToolbarProps) {
  ...
  useSignalEffect(() => {
    ...
    if (!showNavPopup && _resetState === 'confirm') resetState.value = 'idle'
  })
