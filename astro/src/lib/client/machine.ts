import { type Point, createBlot } from '../drawing-toolkit/index'
...
export async function connect() {
  ...
  const inst = await createBlot(port)
...
export async function tryAutoConnect() {
  const p = (await navigator.serial.getPorts()).find(
    p => p.getInfo().usbVendorId === 11914
  )
  if (!p) return
  patchStore({
    inst: await createBlot(p)
  })
}
...
export async function runMachine(scaleX: number = 1, scaleY: number = 1) {
  ...
  try {
    ...
    const goToScaled = async (...[x, y]: Point) =>
      await inst.goTo(x * scaleX, y * scaleY)
