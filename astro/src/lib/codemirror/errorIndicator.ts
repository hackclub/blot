...
const errorEffect = StateEffect.define<{ pos: number | null }>({
    map: (val, mapping) => ({ pos: val.pos === null ? null : mapping.mapPos(val.pos) })
});
...
const errorState = StateField.define<RangeSet<GutterMarker>>({
    ...
    update(set, transaction) {
        ...
        for(const e of transaction.effects) {
            if(e.is(errorEffect)) {
                set = e.value.pos === null ? RangeSet.empty : RangeSet.of([
                    errorMarker.range(e.value.pos)
                ]);
            }
        }
        return set;
...
export const setErrorPos = (view: EditorView, pos: number | null) => view.dispatch({ effects: errorEffect.of({ pos }) });
