...
export const numberScrubbingPlugin = ViewPlugin.fromClass(
  class {
    ...
    eventHandlers: {
      mousedown(e, view) {
        ...
        if (ns) {
          ...
          this.dragging = true
          const numStr = view.state.doc
            .sliceString(this.updateSpan.from, this.updateSpan.to)
            .replaceAll(' ', '') /* handle numbers like `- 5` */
      ...
      mousemove(e, view) {
        ...
        this.num += this.sigfigs
          ? e.movementX * 10 ** (-1 * this.sigfigs)
          : e.movementX
