...
function isect_circ_line(cx: number, cy: number, r: number, x0: number, y0: number, x1: number, y1: number) {
...
export function resample(polyline: Polyline, step: number): Polyline {
    if (polyline.length < 2) {
        return polyline.slice();
    }
    polyline = polyline.slice();
    let out: Polyline = [polyline[0].slice() as Point];
    let next = null;
    let i = 0;
    while (i < polyline.length - 1) {
        ...
        for (let j = i + 2; j < polyline.length; j++) {
            ...
            let q: Point = [b[0] * (1 - t) + c[0] * t, b[1] * (1 - t) + c[1] * t];
    ...
    out.push(polyline[polyline.length - 1].slice() as Point);
    return out;
}
