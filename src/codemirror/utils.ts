...
export const rangeToCodePositions = (
  from: number,
  to: number,
  view: EditorView
): [CodePosition, CodePosition] =>
  [from, to].map(offset => offsetToCodePosition(offset, view)) as [
    CodePosition,
    CodePosition
  ]

export const offsetToCodePosition = (
  offset: number,
  view: EditorView
): CodePosition => {
...
export const codePositionToOffset = (pos: CodePosition, state: EditorState) =>
  state.doc.line(pos.line).from + pos.column
