  ...
  for (let { from, to } of view.visibleRanges) {
    ...
    const enter = cur => {
      ...
      if (cur.name == 'CallExpression') {
        ...
        const fullString = view.state.doc.sliceString(
          fullRange.from,
          fullRange.to
        )
        ...
        CALLNAME_BUTTONS.forEach(cb => {
          ...
          const pathMark = Decoration.mark({
            attributes: {
              ...
              'data-args-end':
                fullRange.from + fullString.indexOf('(') + argsString.length,
              [`data-${callname}`]: ''
            },
          ...
          builder.add(
            fullRange.from,
            fullRange.from + callname.length,
            pathMark
          )
        })
      }
...
export const widgetButtons = ViewPlugin.fromClass(
  class {
    constructor(view) {
