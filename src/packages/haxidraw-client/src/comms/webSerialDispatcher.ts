export class WebSerialDispatcher {
    ...
    constructor(port: SerialPort, handler: (msg: Uint8Array) => any, baudRate: number = 115200) {
        this.#port = port;
    ...
    async close() { this.#keepReading = false; this.#reader?.cancel(); await this.#loopPromise; await this.#port.close(); }
}

export async function createWebSerialDispatcher(port: SerialPort, handler: (msg: Uint8Array) => any, baudRate?: number) {
