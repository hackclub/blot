...
let scaled_cosine = function (i: number) {
    return 0.5 * (1.0 - Math.cos(i * Math.PI));
};

let perlin: Float64Array; // will be initialized lazily by noise() or noiseSeed()

type Vector3 = [number, number, number];

export function noise(vector: number | Vector3, options: Partial<{
    octaves: number,
    falloff: number
}> = {}) {
